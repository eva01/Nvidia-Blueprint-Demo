...
## Turn-Taking and Answer Patience

For structured intake demos, prefer correctness over the lowest possible latency. Disable speculative speech and increase ASR endpointing history so the agent waits through short pauses before it sends an answer to the LLM.

```bash
ENABLE_SPECULATIVE_SPEECH=false
ASR_STOP_HISTORY=900
ASR_STOP_HISTORY_EOU=900
```

`ENABLE_SPECULATIVE_SPEECH=false` prevents stable interim transcripts from triggering the LLM before the final ASR transcript arrives. `ASR_STOP_HISTORY` and `ASR_STOP_HISTORY_EOU` control how much audio history the ASR endpointing logic uses before marking an utterance complete. Higher values make the agent more patient, but they also add a little latency after the user stops speaking.
