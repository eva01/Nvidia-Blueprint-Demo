...
import { Transcripts, type LocalUserMessage } from "./Transcripts";
import { FacilityQuickReplies } from "./FacilityQuickReplies";
...
function App() {
  ...
  const [latestBotText, setLatestBotText] = useState<string>("");
  const [localUserMessages, setLocalUserMessages] = useState<LocalUserMessage[]>([]);
  const [closingTicketId, setClosingTicketId] = useState<string>("");
  ...
  const webRTCStopRef = useRef<(() => void) | null>(null);
  webRTCStopRef.current = "stop" in webRTC ? webRTC.stop : null;
  const closingReturnTimerRef = useRef<number | null>(null);
  const closingFallbackTimerRef = useRef<number | null>(null);

  const clearClosingTimers = useCallback(() => {
    if (closingReturnTimerRef.current !== null) {
      window.clearTimeout(closingReturnTimerRef.current);
      closingReturnTimerRef.current = null;
    }
  ...
  const returnToDashboardAfterTicketClose = useCallback(() => {
    clearClosingTimers();
    webRTCStopRef.current?.();
  ...
  useEffect(() => {
    ...
    const onMessage = (ev: MessageEvent) => {
      try {
        ...
        } else if (payload?.type === "facility_ticket_created") {
          const ticketId = typeof payload.ticket_id === "string" ? payload.ticket_id : "ticket";
          toast.success(`${ticketId} created. Finishing the call.`);
          setClosingTicketId(ticketId);
          setStarted(false);
          setPendingStart(false);
          clearClosingTimers();
          closingFallbackTimerRef.current = window.setTimeout(
            returnToDashboardAfterTicketClose,
            FACILITY_TICKET_CREATED_FALLBACK_STOP_DELAY_MS
          );
        }
      } catch {
        // Ignore malformed control messages from the data channel.
      }
    };
  ...
  useEffect(() => {
    if (!closingTicketId || !isFacilityTicketClosingTranscript(latestBotText, closingTicketId)) return;
    if (closingReturnTimerRef.current !== null) return;
    closingReturnTimerRef.current = window.setTimeout(
      returnToDashboardAfterTicketClose,
      FACILITY_TICKET_CREATED_STOP_DELAY_MS
    );
  }, [closingTicketId, latestBotText, returnToDashboardAfterTicketClose]);

  useEffect(() => clearClosingTimers, [clearClosingTimers]);
  ...
  const handleQuickReply = useCallback((text: string) => {
    ...
    const id = `quick-reply-${Date.now()}`;
    setLocalUserMessages((prev) => [...prev, { id, text, timestamp: new Date() }]);
  ...
  return (
    <div className="h-screen flex flex-col app-shell">
      <Header />
      <nav className="facility-view-nav" aria-label="Primary views">
        <button
      ...
      {activeView === "dashboard" ? (
        <FacilityDashboard onStartVoice={() => setActiveView("voice")} />
      ) : (
        <>
          <section className="flex-1 flex min-h-0">
            <div className="flex-1 p-5 min-w-0">
              {closingTicketId && (
                <div className="facility-call-ending" role="status">
                  <strong>{closingTicketId} created.</strong>
                  <span>The assistant is finishing the closing message, then this will return to ticket review.</span>
                </div>
              )}
              <TechnologyTips compact showFlow />
              <AudioStream
              ...
              <Transcripts
                ...
                onLatestBotText={setLatestBotText}
              />
            </div>
          ...
          {webRTC.status === "connected" && started && (
            <FacilityQuickReplies
              latestBotText={latestBotText}
              disabled={(webRTC as any).dataChannel?.readyState !== "open"}
