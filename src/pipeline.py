...
from nvidia_pipecat.processors.nvidia_rtvi import NvidiaRTVIObserver
...
from src.facility_rtvi import FacilityRTVIInput  # noqa: E402
from src.facility_ticket_processor import FacilityTicketMarkerProcessor  # noqa: E402
from src.facility_ticket_routes import get_ticket_store  # noqa: E402
from src.facility_ticket_routes import router as facility_ticket_router  # noqa: E402
from src.nvidia_rag_providers import NvidiaSafetyGuardProvider  # noqa: E402
from src.safety_guard_processor import SafetyGuardProcessor  # noqa: E402
from src.school_knowledge_base import load_school_knowledge_base  # noqa: E402
from src.school_rag_processor import SchoolRAGContextProcessor  # noqa: E402
...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        logger.warning(f"Invalid {name}, falling back to default {default}")
        return default
...
app.include_router(facility_ticket_router)
...
async def run_bot(webrtc_connection):
    ...
    stt_config = {
        ...
        "stop_history": _env_int("ASR_STOP_HISTORY", 500),
        "stop_history_eou": _env_int("ASR_STOP_HISTORY_EOU", 240),
    }
    if MULTILINGUAL_MODE:
        stt_config.update(
            stop_history=_env_int("ASR_STOP_HISTORY", 900),
            stop_history_eou=_env_int("ASR_STOP_HISTORY_EOU", 900),
        )
    ...
    enable_facility_ticket_autocreate = os.getenv("ENABLE_FACILITY_TICKET_AUTOCREATE", "true").lower() == "true"
    facility_ticket_processor = (
        FacilityTicketMarkerProcessor(get_ticket_store()) if enable_facility_ticket_autocreate else None
    )
    enable_school_knowledge_rag = os.getenv("ENABLE_SCHOOL_KNOWLEDGE_RAG", "true").lower() == "true"
    school_rag_processor = None
    if enable_school_knowledge_rag:
        kb_path = Path(
            os.getenv(
                "SCHOOL_KNOWLEDGE_BASE_PATH",
                str(Path(__file__).parent.parent / "config" / "school_knowledge_base.yaml"),
            )
        )
        school_rag_processor = SchoolRAGContextProcessor(
            load_school_knowledge_base(kb_path),
            min_score=_env_int("SCHOOL_RAG_MIN_SCORE", 45),
        )
    enable_nvidia_safety_guard = os.getenv("ENABLE_NVIDIA_SAFETY_GUARD", "false").lower() == "true"
    safety_guard_processor = (
        SafetyGuardProcessor(
            NvidiaSafetyGuardProvider(timeout=_env_int("NVIDIA_SAFETY_TIMEOUT_SECONDS", 30)),
            fallback_text=os.getenv(
                "NVIDIA_SAFETY_FALLBACK_TEXT",
                "I cannot help with that request. Please ask about school facility support.",
            ),
        )
        if enable_nvidia_safety_guard
        else None
    )
    ...
    pipeline = Pipeline(
        [
            ...
            *([school_rag_processor] if school_rag_processor else []),
            context_aggregator.user(),
            llm,  # LLM
            *([safety_guard_processor] if safety_guard_processor else []),
            *([facility_ticket_processor] if facility_ticket_processor else []),
            tts,  # Text-To-Speech
